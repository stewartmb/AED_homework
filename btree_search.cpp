...
private:
    ...
    void auxTablaBtree(Node* node, Hashmap<int, T>& table, int& index) {
        if (node == nullptr) return;
        auxTablaBtree(node->left, table, index); // Recorrer subárbol izquierdo
        table.insert(index++, node->data);       // Guardar valor en tabla hash con índice
        auxTablaBtree(node->right, table, index); // Recorrer subárbol derecho
    }
...
public:
    ...
    void tabla_btree(Hashmap<int, T>& table) {
        int index = 0;
        auxTablaBtree(root, table, index);
    }
